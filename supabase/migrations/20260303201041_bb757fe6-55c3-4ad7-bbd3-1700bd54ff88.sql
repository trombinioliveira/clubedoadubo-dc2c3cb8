...
-- 1) Add advisory lock to generate_pros_batch for race condition prevention
CREATE OR REPLACE FUNCTION public.generate_pros_batch(p_amount integer, p_user_id uuid)
 RETURNS TABLE(total_generated integer, first_position integer, last_position integer, sample_codes text[])
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  -- Advisory lock to prevent concurrent position calculation
  PERFORM pg_advisory_xact_lock(42424242);

  -- Get starting position (safe under lock)
