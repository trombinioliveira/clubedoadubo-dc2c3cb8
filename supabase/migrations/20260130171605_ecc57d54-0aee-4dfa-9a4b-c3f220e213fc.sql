-- Drop and recreate with unique code generation
...
BEGIN
  ...
  -- Generate all PROs using generate_series with truly unique codes
  -- Using row_number combined with gen_random_uuid ensures uniqueness
  WITH generated_codes AS (
    SELECT 
      gen_random_uuid() as id,
      -- Use first 8 chars of UUID (guaranteed unique) converted to alphanumeric style
      upper(
        translate(
